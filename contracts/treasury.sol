...
interface IBoardroom {
    ...
    function getBaseRatePrice() external view returns (uint256);
...
contract Treasury is ContractGuard {
    ...
    modifier checkEpoch() {
        ...
        epoch = epoch.add(1);
        epochSupplyContractionLeft = (getBaseRatePrice() >
            baseRatePriceCeiling)
            ? 0
        ...
        returns (uint256 _redeemableBonds)
    {
        ...
        if (_baseRatePrice > baseRatePriceCeiling) {
            uint256 _totalBaseRate = IERC20(baseRate).balanceOf(
                address(this)
            );
    ...
    function initialize(
        ...
        baseRatePriceOne = 10 ** 18; // This is to allow a PEG of 1 BRATE per MIM
    ...
    function addToExcludedFromTotalSupply(address _address) public onlyOperator {
        require(!isAdded(_address), "Address already added");
        excludedFromTotalSupply.push(_address);
    }

    function removeFromExcludedFromTotalSupply(address _address) public onlyOperator {
    ...
    function buyBonds(
        ...
        require(
            newBondSupply <=
                baseRateSupply.mul(maxDebtRatioPercent).div(10000),
            "over max debt ratio"
        );
